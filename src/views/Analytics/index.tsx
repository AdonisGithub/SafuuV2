...
const Analytics = () => {
    ...
    useEffect(() => {
        ...
        if(txLength == safuuPriceHistory.length && txLength == bnbPriceHistory.length) {
            let invest = 0, earn = 0;
            for(let i=0; i<txLength; i++){
                if(data[i].get("to") === address.toLocaleLowerCase()) {
                    invest += safuuPriceHistory[i] * Number(data[i].get("value")) / Math.pow(10, 5);
                } else {
                    earn += safuuPriceHistory[i] * Number(data[i].get("value")) / Math.pow(10, 5);
                }
            }
            setInvest(invest);
            setEarn(earn);
        }
    },[safuuPriceHistory,bnbPriceHistory]);
                                        ...
                                        <TableBody >{
                                            txHistory?.map((tx, i) => (
                                                <TableRow key={i}>
                                                    ...
                                                    <TableCell align="center">{trim(safuuPriceHistory[i],3)}</TableCell>
